    ...
    public static class BestiaryReader
    {
        ...
        private static string ResolveContentName(int contentId)
        {
            try
            {
                ...
                return TextUtils.StripIconMarkup(GetGameMessage(mesId));
            }
            catch
            {
                return null;
        ...
        /// Used internally for resolving monster/content names.
        /// </summary>
        private static string GetGameMessage(string mesId)
        {
            if (string.IsNullOrEmpty(mesId)) return null;
            try
            {
