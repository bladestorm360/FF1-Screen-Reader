...
echo Building FFI Screen Reader Mod... > build_log.txt
echo Building...
...
echo Deploying...
call "%DEV_ROOT%\dev_env.bat" find_game "Final Fantasy PR" GAME_ROOT
if not defined GAME_ROOT (
    echo ERROR: "Final Fantasy PR" not found in any Steam library.
    echo   Detected: %STEAM_LIBS%
    exit /b 1
)
set "GAME_DIR=%GAME_ROOT%\Mods"
copy /Y "bin\Debug\net6.0\FFI_ScreenReader.dll" "%GAME_DIR%\" >> build_log.txt 2>&1
